...
class CodingWrappedWriter:
    def __init__(self, coding, writer):
        ...
        self.write = lambda s: writer.write(s)
...
class QQBot:
    def Login(self, qqNum=None):
        ...
        tmpFile.write("开始启动，请扫描二维码登录，可能需要加载大量好友信息，如果网速不好，请耐心等待！！")
        tmpFile.close()
    ...
    def getVfwebqq(self):
        ...
        self.vfwebqq =self.vfwebqq.decode()
        print(self.vfwebqq)
        ...
        result = self.smartRequest(
            ...
            data = {
                'r': json.dumps({
                    ...
                    "psessionid":self.psessionid.decode(), "key":""
                })
            },
        ...
        self.smartRequest(
            ...
            data = {
                'r': json.dumps({
                    ...
                    "msg_id": self.msgId,
                    "psessionid": self.psessionid.decode()
                })
            },
            Referer = 'http://d1.web2.qq.com/proxy.html?v=20151105001&callback=1&id=2'
        )        
    ...
    def smartRequest(self, url, data=None, repeatOnDeny=2, sessionObj=None, **kw):
        ...
        while True:
            ...
            try:
                ...
                elif(isinstance(html,bytes)==True):
                    result = json.loads(html.decode())
                else:
                    result = json.loads(html.decode())
    ...
    def Run(self):
        ...
        tmpFile=open(TmpDir+'/loginstatas.wenq','w')
        tmpFile.write("启动完成！！请向该QQ发送输入的口令开始推送消息。")
        tmpFile.close()
